...
class m220714_095620_create_table_report_by_search_function extends Migration
{
    public function safeUp()
    {
        ...
        $this->createTable(
            ...
            [
                ...
                'project_id' => $this->integer()->notNull()->defaultValue('0')->comment('Primary key of table is auto increment'),
                'agricultural_product_id' => $this->integer()->comment('ລະຫັດສິນຄ້າກະສິກຳ'),
                'mineral_id' => $this->integer()->comment('Is foreign keys from relation table mineral'),
                'energy_type_id' => $this->integer()->comment('Energy type ID is foreign keys from relation table energy type'),
                'control_list_id' => $this->integer()->comment('Control list ID is foreign keys from relation table control list '),
                'ppp_control_list_id' => $this->integer()->comment('Is foreign keys from relation table controlist'),
                'ocs_control_list_id' => $this->integer()->comment('Is foreign keys from relation table control list'),
    ...
    {
        $this->dropTable('{{%report_by_search_function}}');
    }
}
